...
class SimEnv():
                ...
                while reached is not True:
                    ...
                    if True:
                        if (scan >= dist_remaining):
                            reached = True
                            try:
                                self._steps.append([i, dist_remaining[0]])
                            except:
                                self._steps.append([i, dist_remaining])
                        else:
                            if scan > 0.01:
                                xstart += (scan - 0.01)*np.cos(angle_set)
                                ystart += (scan - 0.01)*np.sin(angle_set)
                                dist_remaining -= (scan - 0.01)
                                self._steps.append([i, (scan[0] - 0.01)])
    ...
    def save_result(self):
        ...
        path = f'{base_path}/Sims/Algo-Comp/Room'+self.room_number_str+'_step'+str(int(self._max_step))+'_source'+str(int(self._strength))
        i = 1
        if os.path.exists(path+str(i)+".csv"):
            while os.path.exists(path+str(i)+".csv"):
                i += 1
        with open(path+'_data_run'+str(i)+".csv", 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["iteration", "x", "y", "step_size", "counts"])
            writer.writerows(self._data)
            print("\ndata saved in:", file.name)
        with open(path+'_step_run'+str(i)+".csv", 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["iteration", "step_size"])

...
for x, nb in enumerate(n_bin_list):
    ...
    binDestList = [[] for x in range(sz)]

    xIdxNew = np.max(np.argwhere(xedges < XX[0])).astype(int)
    yIdxNew = np.max(np.argwhere(yedges < YY[0])).astype(int)
    mainIdxNew = int(xIdxNew*(len(yedges)-1)+yIdxNew)

    for row in range(XX.shape[0]-1):
        xIdx = xIdxNew # np.max(np.argwhere(xedges < XX[row])).astype(int)
        yIdx = yIdxNew # np.max(np.argwhere(yedges < YY[row])).astype(int)
        mainIdx = mainIdxNew

        xIdxNew = np.max(np.argwhere(xedges < XX[row+1])).astype(int)
        yIdxNew = np.max(np.argwhere(yedges < YY[row+1])).astype(int)
        mainIdxNew = int(xIdxNew*(len(yedges)-1) + yIdxNew)

        binList[mainIdx].append(row+1)
        binDestList[mainIdx].append(mainIdxNew)
        # Origin information in the length of each list. 
        # Destination information in the appended integers

    denomVec = np.zeros(sz, dtype='int')
    numeratorMatrix = np.zeros((sz, sz), dtype='int')
    for k in range(sz):
        denomVec[k] = len(binDestList[k])
        for i in range(denomVec[k]):
            idx_tmp = binDestList[k][i]
            numeratorMatrix[idx_tmp, k] += 1

    with open('AltData/Room'+room_number_str+'/Pnum_bins'+str(nb)+'_maxstep'+max_step_str+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(numeratorMatrix)
        print("\nData saved:", file.name)

    with open('AltData/Room'+room_number_str+'/Pden_bins'+str(nb)+'_maxstep'+max_step_str+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(denomVec)
        print("\nData saved:", file.name)
    ...
    for ix in range(len(xedges)-1):
        for iy in range(len(yedges)-1):
            ...
            if imageMean(im_base[47:1007,48:1008], [int(x_edge_px[ix]), int(x_edge_px[ix+1])], [int(y_edge_px[iy]), int(y_edge_px[iy+1])]) >= 0.8:
    ...
    with open('AltData/Room'+room_number_str+'/P_bins'+str(nb)+'_maxstep'+max_step_str+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(matrix)
        print("\nData saved:", file.name)

    Omega = matrix #np.genfromtxt('P_room6_bins'+str(nb)+'_maxstep'+max_step_str+'.csv', delimiter=',')
    Tk = np.zeros(sz)
    ...
    np.savetxt('AltData/Room'+room_number_str+'/TK_bins'+str(nb)+'_maxstep'+max_step_str+'.csv', Tk, delimiter=',')
    ...
    expected_times[x] = traversal_time_mean
    upper_times[x] = traversal_time_upper

    np.savetxt('AltData/Room'+room_number_str+'/E_bins'+str(nb)+'_maxstep'+max_step_str+'.csv',[traversal_time_mean],delimiter=',')
...
with open('AltData/Room'+room_number_str+'/ET.csv', writeOrAppendStr, newline='') as file:
    writer = csv.writer(file)
    writer.writerow(expected_times.reshape(1, 3))
    print("\nData saved:", file.name)

with open('AltData/Room'+room_number_str+'/UT.csv', writeOrAppendStr, newline='') as file:
    writer = csv.writer(file)
    writer.writerow(upper_times.reshape(1, 3))
    print("\n Data saved:", file.name)
